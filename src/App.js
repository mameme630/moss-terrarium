...
export default function App() {
  ...
  const startDrag = (e, type, id = null) => {
    ...
    if (type === "avatar") { setDragOffset({ x: pos.x - avatarPos.x, y: pos.y - avatarPos.y }); }
    else { const deco = decorations.find(d => d.id === id); if (deco) setDragOffset({ x: pos.x - deco.x, y: pos.y - deco.y }); }
    setDragging({ type, id });
  };
  ...
  const onMove = useCallback((e) => {
    ...
    const clientY = e.touches ? e.touches[0].clientY : e.clientY;
    const pos = getRelPos(clientX, clientY);
    if (dragging.type === "avatar") { setAvatarPos({ x: Math.max(5, Math.min(95, pos.x - dragOffset.x)), y: Math.max(10, Math.min(90, pos.y - dragOffset.y)) }); }
    else { setDecorations(ds => ds.map(d => d.id === dragging.id ? { ...d, x: Math.max(2, Math.min(96, pos.x - dragOffset.x)), y: Math.max(5, Math.min(92, pos.y - dragOffset.y)) } : d)); }
  }, [dragging, dragOffset]);
  ...
  useEffect(() => {
    ...
    return () => { window.removeEventListener("mousemove", onMove); window.removeEventListener("mouseup", endDrag); window.removeEventListener("touchmove", onMove); window.removeEventListener("touchend", endDrag); };
  }, [onMove, endDrag]);

  const addDeco = (emoji) => { setDecorations(ds => [...ds, { id: nextId++, emoji, x: 30 + Math.random() * 40, y: 50 + Math.random() * 30, scale: 0.8 + Math.random() * 0.6 }]); };
      ...
      <div style={{ display: "flex", gap: "16px", width: "100%", maxWidth: "960px", flexWrap: "wrap", justifyContent: "center" }}>
        <div style={{ position: "relative", flex: "1 1 300px", maxWidth: "520px" }}>
          <div style={{ position: "relative", borderRadius: "48% 48% 30% 30% / 20% 20% 10% 10%", overflow: "hidden", aspectRatio: "3/4", background: theme.bg, border: "2px solid rgba(200,255,200,0.15)", boxShadow: "0 0 60px rgba(74,222,128,0.1), inset 0 0 80px rgba(0,0,0,0.5), 0 20px 60px rgba(0,0,0,0.8)", cursor: dragging ? "grabbing" : "default", userSelect: "none" }} ref={terrRef}>
            ...
            {sparkles.map(sp => (<div key={sp.id} style={{ position: "absolute", left: sp.x, top: sp.y, pointerEvents: "none", zIndex: 20, fontSize: "20px", animation: "sparkleUp 0.8s forwards", transform: "translate(-50%, -50%)" }}>✨</div>))}
        ...
        <div style={{ flex: "1 1 200px", maxWidth: "280px", display: "flex", flexDirection: "column", gap: "10px" }}>
          <div style={{ display: "flex", gap: "4px", background: "rgba(255,255,255,0.05)", borderRadius: "12px", padding: "4px" }}>
            {[{ id: "theme", label: "🎨 テーマ" }, { id: "avatar", label: "👤 自分" }, { id: "deco", label: "🍄 飾り" }].map(tab => (<button key={tab.id} onClick={() => setShowPanel(tab.id)} style={{ flex: 1, padding: "6px 2px", borderRadius: "8px", border: "none", background: showPanel === tab.id ? theme.accent : "transparent", color: showPanel === tab.id ? "#000" : "#9ca3af", fontWeight: 700, fontSize: "11px", cursor: "pointer", transition: "all 0.2s" }}>{tab.label}</button>))}
          </div>
          {showPanel === "theme" && (<div style={{ display: "flex", flexDirection: "column", gap: "8px" }}><p style={{ color: "#9ca3af", fontSize: "12px", margin: 0 }}>テーマを選んでね</p>{TERRARIUM_THEMES.map(t => (<button key={t.id} onClick={() => setTheme(t)} style={{ padding: "10px 14px", borderRadius: "12px", border: `2px solid ${theme.id === t.id ? t.accent : "rgba(255,255,255,0.1)"}`, background: theme.id === t.id ? `${t.accent}22` : "rgba(255,255,255,0.05)", color: theme.id === t.id ? t.accent : "#9ca3af", fontWeight: 700, fontSize: "13px", cursor: "pointer", textAlign: "left", transition: "all 0.2s" }}>{t.name}</button>))}</div>)}
          {showPanel === "avatar" && (<div style={{ display: "flex", flexDirection: "column", gap: "12px" }}><p style={{ color: "#9ca3af", fontSize: "12px", margin: 0 }}>写真と名前を設定しよう</p><div onClick={() => fileRef.current.click()} style={{ border: `2px dashed ${theme.accent}66`, borderRadius: "16px", padding: "20px", textAlign: "center", cursor: "pointer", background: `${theme.accent}11` }}>{avatarSrc ? <img src={avatarSrc} style={{ width: 56, height: 56, borderRadius: "50%", objectFit: "cover", border: `2px solid ${theme.accent}` }} alt="preview" /> : <div style={{ fontSize: "30px" }}>📷</div>}<p style={{ color: theme.accent, fontSize: "12px", margin: "6px 0 0", fontWeight: 700 }}>{avatarSrc ? "タップで変更" : "写真をアップ"}</p><input ref={fileRef} type="file" accept="image/*" onChange={handleUpload} style={{ display: "none" }} /></div><div><label style={{ color: "#9ca3af", fontSize: "12px", display: "block", marginBottom: "6px" }}>表示名</label><input value={avatarName} onChange={e => setAvatarName(e.target.value)} maxLength={10} style={{ width: "100%", padding: "8px 12px", borderRadius: "10px", border: `1px solid ${theme.accent}44`, background: "rgba(255,255,255,0.07)", color: "#fff", fontSize: "14px", outline: "none", boxSizing: "border-box" }} /></div><div><label style={{ color: "#9ca3af", fontSize: "12px", display: "block", marginBottom: "6px" }}>サイズ: {Math.round(avatarScale * 100)}%</label><input type="range" min="0.4" max="2.0" step="0.1" value={avatarScale} onChange={e => setAvatarScale(parseFloat(e.target.value))} style={{ width: "100%", accentColor: theme.accent }} /></div></div>)}
          {showPanel === "deco" && (<div style={{ display: "flex", flexDirection: "column", gap: "10px" }}><p style={{ color: "#9ca3af", fontSize: "12px", margin: 0 }}>クリックで追加・ダブルクリックで削除</p><div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: "8px" }}>{theme.decorations.map((emoji, i) => (<button key={i} onClick={() => addDeco(emoji)} style={{ background: "rgba(255,255,255,0.07)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "12px", padding: "8px 0", fontSize: "20px", cursor: "pointer", color: "white" }}>{emoji}</button>))}</div><p style={{ color: "#6b7280", fontSize: "11px", margin: 0, textAlign: "center" }}>現在 {decorations.length} 個配置中</p><button onClick={() => setDecorations([])} style={{ padding: "8px", borderRadius: "10px", border: "1px solid rgba(239,68,68,0.3)", background: "rgba(239,68,68,0.1)", color: "#ef4444", fontSize: "12px", cursor: "pointer", fontWeight: 700 }}>🗑️ 全部リセット</button></div>)}
